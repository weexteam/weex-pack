...
var _buildOnRealDevice = function _buildOnRealDevice(_ref11) {
  ...
  try {
    resolve({ device: device, xcodeProject: xcodeProject, options: options, rootPath: rootPath });
  } catch (e) {
    reject(e);
  }
  resolve({ device: device, xcodeProject: xcodeProject, options: options, configs: configs });
...
var _runAppOnDevice = function _runAppOnDevice(_ref14) {
  ...
  var appPath = 'build/Debug-iphoneos/WeexDemo.app';
  var deviceId = device.udid;
  try {
    if (!fs.existsSync(appPath)) {
      logger.info('building...');
      childprocess.execSync(path.join(__dirname, '../build/lib/cocoapods-build') + ' . Debug', { encoding: 'utf8' });
    }
