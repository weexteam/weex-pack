...
var _buildOnRealDevice = function _buildOnRealDevice(_ref11) {
  ...
  try {
    ...
    childprocess.execSync('xcodebuild -' + (xcodeProject.isWorkspace ? 'workspace' : 'project') + ' ' + xcodeProject.name + ' -scheme ' + scheme + ' -configuration Debug -destination id=' + device.udid + ' -derivedDataPath build' + ' clean build \'CODE_SIGN_IDENTITY=' + configs.CodeSign + '\'', {
      encoding: 'utf8'
    });
  } catch (e) {
    reject(e);
...
var _runAppOnDevice = function _runAppOnDevice(_ref14) {
  ...
  if (options.iosBuildPath && fs.existsSync(options.iosBuildPath)) {
    ...
    reject('You may had custome your Xcode Derived Data path, please use `--iosBuildPath` to set your path.');
    return;
  }
