...
var libPath = path.join(globalConfigPath, 'lib');

exports.binname = 'weex';
exports.globalConfig = globalConfigPath;
...
function cdProjectRoot(dir) {
  ...
  if (!projectRoot) {
    throw new CordovaError('Current working directory is not a weex project.');
  }
  if (!origCwd) {
...
function listPlatforms(projectDir) {
  ...
  if (!exports.existsSync(platformsDir)) {
    return [];
  }
...
function getInstalledPlatformsWithVersions(projectDir) {
  ...
  return Q.all(platforms.map(function (p) {
    ...
    return superspawn.maybeSpawn(path.join(projectDir, 'platforms', p, 'cordova', 'version'), [], { chmod: true }).then(function (v) {
      result[p] = v || null;
    }, function (v) {
      result[p] = 'broken';
    });
...
function ensurePlatformOptionsCompatible(platformOptions) {
  ...
  events.emit('warn', 'The format of cordova.raw.* methods "options" argument was changed in 5.4.0. ' + '"options.options" property now should be an object instead of an array of plain strings. Though the old format ' + 'is still supported, consider updating your cordova.raw.* method calls to use new argument format.');

  var knownArgs = ['debug', 'release', 'device', 'emulator', 'nobuild', 'list', 'buildConfig', 'target', 'archs'];

  opts = nopt({}, {}, opts, 0);
  opts.argv = Object.keys(opts).filter(function (arg) {
    return arg !== 'argv' && knownArgs.indexOf(arg) === -1;
  }).map(function (arg) {
    return opts[arg] === true ? '--' + arg : '--' + arg + '=' + opts[arg].toString();
  });
...
// optWrap is a boolean: True means that a callback-based wrapper for the promise-based function
// should be created.
function addModuleProperty(module, symbol, modulePath, optWrap, optObj) {
  var val = null;
  if (optWrap) {
    ...
    Object.defineProperty(optObj || module.exports, symbol, {
      get: function get() {
        val = val || module.require(modulePath);return val;
      },
      set: function set(v) {
        val = v;
      }
 ...
 * @returns {Promise} Promise for an array of versions.
 */
function getLatestNpmVersion(moduleName) {
  var npm = require('npm');
  return Q.nfcall(npm.load).then(function () {
    return Q.ninvoke(npm.commands, 'view', [moduleName, 'version'], /* silent = */true).then(function (result) {
      // result is an object in the form:
