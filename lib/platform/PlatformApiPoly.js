...
var superspawn = weexpackCommon.superspawn;
var xmlHelpers = weexpackCommon.xmlHelpers;
var CordovaError = weexpackCommon.CordovaError;
var PluginInfo = weexpackCommon.PluginInfo;
var ConfigParser = weexpackCommon.ConfigParser;
var PlatformJson = weexpackCommon.PlatformJson;
var ActionStack = weexpackCommon.ActionStack;
var PlatformMunger = weexpackCommon.ConfigChanges.PlatformMunger;
var PluginInfoProvider = weexpackCommon.PluginInfoProvider;
...
PlatformApiPoly.createPlatform = function (destinationDir, projectConfig, options) {
  if (!options || !options.platformDetails) {
    return Q.reject(new CordovaError('Failed to find platform\'s \'create\' script. ' + 'Either \'options\' parameter or \'platformDetails\' option is missing'));
  }
...
PlatformApiPoly.prototype.prepare = function (cordovaProject, options) {
  ...
  xmlHelpers.mergeXml(cordovaProject.projectConfig.doc.getroot(), this._config.doc.getroot(), this.platform, true);
  this._config.write();
...
PlatformApiPoly.prototype.addPlugin = function (plugin, installOptions) {
  if (!plugin || !(plugin instanceof PluginInfo)) {
    return Q.reject('The parameter is incorrect. The first parameter ' + 'should be valid PluginInfo instance');
  }
  ...
  plugin.getFilesAndFrameworks(this.platform).concat(plugin.getAssets(this.platform)).concat(plugin.getJsModules(this.platform)).forEach(function (item) {
    actions.push(actions.createAction(self._getInstaller(item.itemType), [item, plugin.dir, plugin.id, installOptions, projectFile], self._getUninstaller(item.itemType), [item, plugin.dir, plugin.id, installOptions, projectFile]));
  });
  ...
  return actions.process(this.platform, this.root).then(function () {
    ...
    .add_plugin_changes(plugin, installOptions.variables, /* is_top_level=*/true, /* should_increment=*/true).save_all();

    var targetDir = installOptions.usePlatformWww ? self.getPlatformInfo().locations.platformWww : self.getPlatformInfo().locations.www;
...
PlatformApiPoly.prototype.removePlugin = function (plugin, uninstallOptions) {
  ...
  plugin.getFilesAndFrameworks(this.platform).concat(plugin.getAssets(this.platform)).concat(plugin.getJsModules(this.platform)).forEach(function (item) {
    actions.push(actions.createAction(self._getUninstaller(item.itemType), [item, plugin.dir, plugin.id, uninstallOptions, projectFile], self._getInstaller(item.itemType), [item, plugin.dir, plugin.id, uninstallOptions, projectFile]));
  });
  ...
  return actions.process(this.platform, this.root).then(function () {
    ...
    // Ignore passed `is_top_level` option since platform itself doesn't know
    // anything about managing dependencies - it's responsibility of caller.
    .remove_plugin_changes(plugin, /* is_top_level=*/true).save_all();

    var targetDir = uninstallOptions.usePlatformWww ? self.getPlatformInfo().locations.platformWww : self.getPlatformInfo().locations.www;
...
PlatformApiPoly.prototype.updatePlugin = function (plugin, updateOptions) {
  ...
  return this.removePlugin(plugin, updateOptions).then(function () {
...
PlatformApiPoly.prototype.requirements = function () {
  ...
  try {
    ...
    var errorMsg = 'Failed to check requirements for ' + this.platform + ' platform. ' + 'check_reqs module is missing for platform. Skipping it...';
...
PlatformApiPoly.prototype._writePluginModules = function (targetDir) {
  ...
  var finalContents = 'cordova.define(\'cordova/plugin_list\', function(require, exports, module) {\n';
...
PlatformApiPoly.prototype._getInstaller = function (type) {
  ...
  return function (item, pluginDir, pluginId, options, project) {
    ...
    var wwwDest = options.usePlatformWww ? self.getPlatformInfo().locations.platformWww : self._handler.www_dir(self.root);

    var installerArgs = type === 'asset' ? [wwwDest] : type === 'js-module' ? [pluginId, wwwDest] : [self.root, pluginId, options, project];
...
PlatformApiPoly.prototype._getUninstaller = function (type) {
  ...
  return function (item, pluginDir, pluginId, options, project) {
    ...
    var uninstallerArgs = type === 'asset' || type === 'js-module' ? [wwwDest, pluginId] : [self.root, pluginId, options, project];
