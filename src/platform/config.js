...
config.read = function get_config(project_root) {
    let data = configCache[project_root];
    if (!data) {
        const configPath = path.join(project_root, '.wx', 'config.json');
...
config.write = function set_config(project_root, json) {
    let configPath = path.join(project_root, '.wx', 'config.json');
    let contents = JSON.stringify(json, null, 4);
    configCache[project_root] = contents;
    // Don't write the file for an empty config.
    if (contents != '{}' || fs.existsSync(configPath)) {
        shell.mkdir('-p', path.join(project_root, '.wx'));
