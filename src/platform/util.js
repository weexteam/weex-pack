...
const libPath = path.join(globalConfigPath, 'lib');
...
function cdProjectRoot (dir) {
  const projectRoot = this.isCordova(dir);
  if (!projectRoot) {
...
function getInstalledPlatformsWithVersions (projectDir) {
  ...
  return Q.all(platforms.map(function (p) {
    ...
    return superspawn.maybeSpawn(path.join(projectDir, 'platforms', p, 'cordova', 'version'), [], { chmod: true })
        .then(function (v) {
          result[p] = v || null;
        }, function (v) {
          result[p] = 'broken';
