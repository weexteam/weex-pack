...
function runAndroid(options) {
  ...
  }).then(prepareAndroid).then(resolveConfig).then(findAndroidDevice).then(chooseDevice).then(reverseDevice).then(buildApp).then(installApp).then(runApp).catch((err) => {
    if (err) {
...
function prepareAndroid({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!utils.checkAndroid(rootPath)) {
      ...
      console.log();
      console.log(`  You should run ${chalk.blue('weex create')} or ${chalk.blue('weex platform add android')}  first`);
      reject();
    }
    console.log();
...
function reverseDevice({
  ...
  return new Promise((resolve, reject) => {
    try {
      const s = child_process.execSync(`adb -s ${device} reverse tcp:8080 tcp:8080`, {
        encoding: 'utf8'
      });
    }
    catch (e) {
