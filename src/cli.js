...
let path = require('path'),
  ...
  Q = require('q');
const { prefix } = require('./utils/npm');
...
function cli (inputArgs) {
  ...
  if (cmdList.indexOf(cmd) >= 0) {
    ...
    if (args.variable) {
      args.variable.forEach(function (s) {
                // CB-9171
        const eq = s.indexOf('=');
        if (eq === -1) { throw new WeexpackError('invalid variable format: ' + s); }
        const key = s.substr(0, eq).toUpperCase();
        const val = s.substr(eq + 1, s.length);
        cli_vars[key] = val;
      });
    }
    const downloadOpts = { searchpath: args.searchpath,
  ...
  function create () {
    ...
    if (undashed[4]) { cfg = JSON.parse(undashed[4]); }
    else { cfg = {}; }
