...
const superspawn = require('weexpack-common').superspawn;
const semver = require('semver');
const shell = require('shelljs');
const _ = require('underscore');
const PlatformJson = require('weexpack-common').PlatformJson;
...
//   var xml = cordova_util.projectConfig(projectRoot);
  ...
  const autosave = config_json.auto_save_platforms || false;
  if (!targets || !targets.length) {
    msg = 'No platform specified. Please specify a platform to ' + cmd + '. ' + 'See `' + cordova_util.binname + ' platform list`.';
  ...
  return hooksRunner.fire('before_platform_' + cmd, opts).then(function () {
    return promiseutil.Q_chainmap(targets, function (target) {
      ...
      return Q.when().then(function () {
        ...
        if (cmd == 'add') {
          ...
          if (!platformAlreadyAdded) {
            throw new CordovaError('Platform "' + platform + '" is not yet added. See `' + cordova_util.binname + ' platform list`.');
          }
        }
        // if (/-nightly|-dev$/.exec(platDetails.version)) {
        //   msg = 'Warning: using prerelease platform ' + platform + '@' + platDetails.version + '.\nUse \'weex platform add ' + platform + '@latest\' to add the latest published version instead.';
        ...
        if (config_json && config_json.lib && config_json.lib[platform] && config_json.lib[platform].template) {
        ...
        //     }
        // }
        const PlatformApi = require('../platforms/PlatformApiPoly');
...
function remove(hooksRunner, projectRoot, targets, opts) {
  ...
  return hooksRunner.fire('before_platform_rm', opts).then(function () {
...
function check(hooksRunner, projectRoot) {
  ...
  superspawn.spawn('npm', ['--loglevel=silent', '--json', 'outdated', 'cordova-lib'], {
  ...
  cordova.raw.create(scratch).then(function () {
    ...
    Q.all(platforms_on_fs.map(function (p) {
      ...
      if (platformsText) {
        results = platformsText.filter(function (p) {
          return !!p;
        }).sort().join('\n');
      }
      if (!results) {
...
function list(hooksRunner, projectRoot, opts) {
  return hooksRunner.fire('before_platform_ls', opts).then(function () {
    ...
    if (targets) {
      ...
      targets.forEach(function (t) {
        ...
        if (/[~:/\\.]/.test(t)) {
          ...
          msg = 'Platform "' + t + '" not recognized as a core weexpack platform. See `' + cordova_util.binname + ' platform list`.';
        }
